...
function App() {
  ...
  return(
    <div className="estilo">
      <h1>React Formularios - Estudiante</h1>
      {submitting &&
       <div>
         Mostrando registro:
         <ul>
         ...
         <ul>
           {Object.entries(formData4).map(([name, value]) => (
             <li key={name}><strong>Direcciòn</strong>: {value.toString()}</li>
           ))}
         </ul>
         <ul>
           {Object.entries(formData5).map(([name, value]) => (
             <li key={name}><strong>Teléfono</strong>: {value.toString()}</li>
           ))}
         </ul>
       </div>
      }
      <form onSubmit={handleSubmit}>
        <fieldset>
          ...
          <label>
            ...
            <input name="fecnac" onChange={handleChange3}/>
          </label>
          <label>
            <p>Dirección</p>
            <input name="direc" onChange={handleChange4}/>
          </label>
          <label>
            <p>Teléfono</p>
